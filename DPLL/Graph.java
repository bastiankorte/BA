...
public class Graph {
	...
	public boolean assignGRASP(Variable variable, int assignmentInt, int level, Clause ucpClause) {
		//check if variable is already assigned with diverging assignment
		if(variable.getAssignmentInt()*(-1) == assignmentInt) return false;
		else if(variable.getAssignmentInt() == assignmentInt) return true;

		//Assignment assignment = new Assignment(variable, assignmentInt, level, ucpClause);
		variable.assignGRASP(assignmentInt, level, ucpClause);
		for(int i = 0; i < variable.getIncidences().size(); i++) {
			if(variable.getIncidences().get(i).getSign() == assignmentInt) variable.getIncidences().get(i).getClause().increaseSatisfiedVariables();
			//if(variable.getIncidences().get(i).getClause().getIndex() == 28) System.out.println("increased to clause 28");
			variable.getIncidences().get(i).getClause().increaseAssignedVariables();
		}
		assignedAmount++;
		return true;
	}

	public Variable unassignGrasp(Variable variable) {
		//check if variable is already assigned with diverging assignment
		if(variable.getAssignmentInt() == 0) return variable;
		//Assignment assignment = new Assignment(variable, assignmentInt, level, ucpClause);
		for(int i = 0; i < variable.getIncidences().size(); i++) {
			if(variable.getIncidences().get(i).getSign() == variable.getAssignmentInt()) variable.getIncidences().get(i).getClause().decreaseSatisfiedVariables();
			//if(variable.getIncidences().get(i).getClause().getIndex() == 28) System.out.println("increased to clause 28");
			variable.getIncidences().get(i).getClause().decreaseAssignedVariables();
		}
		if(info) System.out.println("unassigned " + variable.toString());
		variable.unassignGrasp();
		assignedAmount--;
		return variable;
	}
	...
	public boolean assignDPLL(Variable variable, int assignmentInt, int level, Clause ucpClause) {
		//check if variable is already assigned with diverging assignment
		if(variable.getAssignmentInt()*(-1) == assignmentInt) return false;
		else if(variable.getAssignmentInt() == assignmentInt) return true;
	...
	public Variable unassignDPLL(Variable variable) {
		if(variable.getAssignmentInt() == 0) return variable;
		for(int i = 0; i < variable.getIncidences().size(); i++) {
			if(variable.getIncidences().get(i).getSign() == variable.getAssignmentInt()) variable.getIncidences().get(i).getClause().decreaseSatisfiedVariables();
			//if(variable.getIncidences().get(i).getClause().getIndex() == 28) System.out.println("increased to clause 28");
	...
	public boolean isSATGRASP() {
		for ( Clause clause : clauses) {
			if(clause.getSatisfiedVariables() == 0) return false;
		}
		return true;
	}

	public boolean isSATDPLL() {
		loop : for (Clause clause : clauses) {
			for(Edge e : clause.getIncidences()) {
				if(e.getSign() == e.getVariable().getAssignmentInt()) continue loop;
			}
