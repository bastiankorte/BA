...
public class Variable {
	...
	public boolean isContradiction() {
		for (int i = 0; i < incidences.size(); i++) {
			if(!incidences.get(i).getClause().isUnitClause() && incidences.get(i).getClause().getIncidences().length > 1) continue;
			for (int j = i + 1; j < incidences.size(); j++) {
				if(!incidences.get(j).getClause().isUnitClause() && incidences.get(j).getClause().getIncidences().length > 1) continue;
	...
	public void flipAssignmentForClauseLearning() { assignmentInt = assignmentInt * (-1); }
	...
	public boolean assignGRASP(int setAssignmentInt, int setLevel, Clause setAntecedent) { 
		if(setAssignmentInt*(-1) == assignmentInt) return false; 
		assignmentInt = setAssignmentInt; 
		antecedent = setAntecedent;
		if(antecedent != null && antecedent.getMaxDepth() == setLevel && setLevel != level) ucplevel = antecedent.getMaxUCPLevel() + 1;
		else ucplevel = 0;
		level = setLevel;
		return true;
	}
	...
	public void unassignGrasp() { assignmentInt = 0; antecedent = null; ucplevel = -1; }
